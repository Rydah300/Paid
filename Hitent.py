...
I have analyzed the script. The `SyntaxError: Invalid Decimal literal` usually occurs in Python if there are invisible hidden characters (often copied from web formatting) or if the encoding of the file is incorrect.

I have rewritten the entire script cleanly to ensure **no syntax errors** remain. I have also added a specific `try-except` block for the username handling to prevent crashes if a user does not have a username.

Here is the fixed, clean code.
